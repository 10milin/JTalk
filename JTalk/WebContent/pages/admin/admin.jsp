	    ...
	    <div class="row">
            ...
            <div class="box-body box-body-padding">
              ...
              <table class="table table-condensed table-hover table-md">
                ...
                <c:if test="${empty currentList}">
                	<c:if test="${empty search}">
	                	<td colspan="5" align="center">관리자가 존재하지 않습니다.</td>
	                </c:if>
                ...
                <c:if test="${not empty currentList}">
                	...
                	<c:forEach var="item" items="${currentList}" varStatus="status">
                		<tr class="table-field" onclick="javascript:showmember('${item.email}')" style="cursor:pointer;">
		                  <td>${item.period}</td>
		                  <td>${item.name}</td>
		                  <td>${item.email}</td>
		                  <td>
              ...
              <table class="table table-condensed table-hover table-xd">
                ...
                <c:if test="${empty currentList}">
                	<c:if test="${empty search}">
	                	<td colspan="4" align="center">관리자가 존재하지 않습니다.</td>
	                </c:if>
	                <c:if test="${not empty search}">
              ...
              <div class="text-right table-bottom">
              	<form action = "/JTalk/admin.action?command=admin" method="post">
              	<div class="col-md-3 col-xs-12 no-padding mobile-center">
              		<c:if test="${not empty search}">
	                    ...
	                    <input type="text" class="form-control" placeholder="이름으로 검색하기" value="${search}" name="search" required>
              ...
              <div class="col-md-9 col-xs-12 text-right no-padding">
              		<c:if test="${empty search}">
              			...
              			<button type="button" class="btn btn-default" onclick="actionlink('admin.action?command=admin');"><i class="glyphicon glyphicon-refresh"></i> 새로고침</button>
                	</c:if>
                	<c:if test="${not empty search}">
                		<button type="button" class="btn btn-default" onclick="searchbar(this);"><i class="glyphicon glyphicon-search"></i> 검색</button>
                		<button type="button" class="btn btn-default" onclick="actionlink('admin.action?command=admin');"><i class="glyphicon glyphicon-refresh"></i> 새로고침</button>
              ...
              <div class="col-md-12 text-center" style="display:inline-block; width:100%">
              	<form action="/JTalk/admin.action?command=admin" method="post" id="pagination-form">
