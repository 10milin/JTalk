            ...
            <div class="col-md-12">
              <table class="table table-condensed table-hover">
                <tr class="table-field board-headline">
                  <th>[<c:out value="${exchange.period}"/>기] [<c:out value="${exchange.category}"/>] <c:out value="${exchange.title}"/></th>
                </tr>
                <tr class="board-content board-white">
                	<td>
                		<i class="fa fa-user"></i> <a href="javascript:showmember('<c:out value="${exchange.writerId}"/>')"><c:out value="${exchange.writerName}"/></a><span style="margin:0 10px;"></span>
                		<fmt:formatDate var="date" value="${exchange.writeDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
                		<i class="fa fa-clock-o"></i> <c:out value="${date}"/><span style="margin:0 10px;"></span>
                		<i class="fa fa-eye"></i> <c:out value="${exchange.hit}"/>
                	</td>
                </tr>
                <tr class="board-white">
                	<td>
                		<div><c:out value="${exchange.content}" escapeXml="false"/></div>
                	</td>
                </tr>
                <c:if test="${not empty exchange.fileName}">
                <tr class="board-white">
                	<td>
                		<div class="col-md-4 col-sm-4 col-xs-12" style="padding:10px 0px;">
			                  <div class="mailbox-attachment-info">
			                    <span class="mailbox-attachment-name"><i class="fa fa-paperclip"></i> <c:out value="${exchange.originFileName}"/></span>
			                          <a href="javascript:actiondownload('exchange.action?command=download','<c:out value="${exchange.fileName}"/>', '<c:out value="${exchange.originFileName}"/>');" class="btn btn-default btn-xs pull-right"><i class="glyphicon glyphicon-download-alt"></i></a>
			                  </div>
                		</div>
                	</td>
                </tr>
		                ...
		                <div class="comment-text">
		                      <span class="username">
		                        ...
		                        <c:if test="${item.writerId eq member.email || member.active ge 2}">
			                        <span class="margin-right-left"><a class="color-black" onclick="actioncmtdelete('comment.action?command=delete', 'exchange', <c:out value="${item.num}"/>);" style="cursor:pointer;"><i class="fa fa-trash"></i></a></span>
		                        </c:if>                   
              ...
              <form action="/JTalk/comment.action?command=write" method="post">
                ...
                <div class="img-push input-group">
                  <input type="hidden" name = "tableName" value = "exchange"/>
                  <input type="hidden" name = "postNum" value = "<c:out value="${exchange.num}"/>"/>
              ...
              <div class="text-right">
              	...
              	<c:if test="${member.email == exchange.writerId}">
	              	<button type="button" class="btn btn-default" onclick="actionparam('exchange.action?command=modifyform',<c:out value="${exchange.num}"/>);"><i class="fa fa-pencil"></i> 수정</button>
              	</c:if>
              	<c:if test="${member.email == exchange.writerId || member.active ge 2}">
              		<button type="button" class="btn btn-default" onclick="actionparam('exchange.action?command=delete',<c:out value="${exchange.num}"/>);"><i class="fa fa-trash"></i> 삭제</button>
              	</c:if>
              </div>
            </div>
            <!-- /.box-body -->
