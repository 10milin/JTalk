            ...
            <div class="col-md-12">
              <table class="table table-condensed table-hover">
                <tr class="table-field board-headline">
                  <th><c:out value="${japanese.title}"/></th>
                </tr>
                <tr class="board-content board-white">
                	<td>
                		<i class="fa fa-user"></i> <a href="javascript:showmember('<c:out value="${japanese.writerId}"/>')"><c:out value="${japanese.writerName}"/></a><span style="margin:0 10px;"></span>
                		...
                		<i class="fa fa-eye"></i> <c:out value="${japanese.hit}"/>
                	</td>
                </tr>
                <tr class="board-white">
                	<td>
                		<div><c:out value="${japanese.content}" escapeXml="false"/></div>
                	</td>
                </tr>
                	...
                	<td>
                		<div class="col-md-4 col-sm-4 col-xs-12" style="padding:10px 0px;">
			                  <div class="mailbox-attachment-info">
			                    <span class="mailbox-attachment-name"><i class="fa fa-paperclip"></i> <c:out value="${japanese.originFileName}"/></span>
			                          <a href="javascript:actiondownload('japanese.action?command=download','<c:out value="${japanese.fileName}"/>', '<c:out value="${japanese.originFileName}"/>');" class="btn btn-default btn-xs pull-right"><i class="glyphicon glyphicon-download-alt"></i></a>
			                  </div>
                		</div>
                	</td>
                </tr>
		                ...
		                <div class="comment-text">
		                      <span class="username">
		                        ...
		                        <c:if test="${item.writerId eq member.email || member.active ge 2}">
			                        <span class="margin-right-left"><a class="color-black" onclick="actioncmtdelete('comment.action?command=delete', 'japanese', <c:out value="${item.num}"/>);" style="cursor:pointer;"><i class="fa fa-trash"></i></a></span>
		                        </c:if>                   
              ...
              <form action="/JTalk/comment.action?command=write" method="post">
                ...
                <div class="img-push input-group">
                  ...
                  <input type="hidden" name = "postNum" value = "<c:out value="${japanese.num}"/>"/>
              ...
              <div class="text-right">
              	...
              	<c:if test="${member.email == japanese.writerId}">
	              	<button type="button" class="btn btn-default" onclick="actionparam('japanese.action?command=modifyform',<c:out value="${japanese.num}"/>);"><i class="fa fa-pencil"></i> 수정</button>
              	</c:if>
              	<c:if test="${member.email == japanese.writerId || member.active ge 2}">
              		<button type="button" class="btn btn-default" onclick="actionparam('japanese.action?command=delete',<c:out value="${japanese.num}"/>);"><i class="fa fa-trash"></i> 삭제</button>
              	</c:if>
              </div>
            </div>
            <!-- /.box-body -->
