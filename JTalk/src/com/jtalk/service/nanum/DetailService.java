...
public class DetailService implements Service {
	...
	public String process(HttpServletRequest request, HttpServletResponse response) {
		...
		for(int i = 0; i < cookie.length; i++) {
			if(cookie[i].getName().equals("nanum" + String.valueOf(num))) {
		...
		if(!check) {
			...
			Cookie noticeCookie = new Cookie("nanum" + String.valueOf(num), String.valueOf(num));
		...
		//게시글과 댓글을 읽어옴
		NanumDTO trade = nanumDAO.getNanum(num);
		ArrayList<CommentDTO> commentList = commentDAO.getCommentList("nanum", num);
		ArrayList<String> profileList = new ArrayList<String>();
		
		//글쓴이가 글을 읽은 경우 새 댓글 알림 초기화
		if(member.getEmail().equals(trade.getWriterID())) {
			newDAO.resetNew("nanum", num);
		}
		...
		//댓글의 총 갯수
		int countComment = commentDAO.countComment("nanum", num);
