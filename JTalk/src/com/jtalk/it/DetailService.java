...
public class DetailService implements Service {
	...
	public String process(HttpServletRequest request, HttpServletResponse response) {
		...
		for(int i = 0; i < cookie.length; i++) {
			if(cookie[i].getName().equals("it" + String.valueOf(num))) {
		...
		if(!check) {
			...
			Cookie noticeCookie = new Cookie("it" + String.valueOf(num), String.valueOf(num));
		...
		//게시글과 댓글을 읽어옴
		ItDTO notice = itDAO.getIt(num);
		ArrayList<CommentDTO> commentList = commentDAO.getCommentList("it", num);
		ArrayList<String> profileList = new ArrayList<String>();
		
		//글쓴이가 글을 읽은 경우 새 댓글 알림 초기화
		if(member.getEmail().equals(notice.getWriterId())) {
			newDAO.resetNew("it", num);
		}
		...
		//댓글의 총 갯수
		int countComment = commentDAO.countComment("it", num);
