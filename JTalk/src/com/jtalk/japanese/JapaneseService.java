...
public class JapaneseService implements Service{
	...
	public String process(HttpServletRequest request, HttpServletResponse response) {
		...
		ArrayList<JapaneseDTO> noticeList; //전체 목록
		ArrayList<JapaneseDTO> currentList = new ArrayList<JapaneseDTO>(); //현재 페이지 목록
		ArrayList<Integer> countList = new ArrayList<Integer>();
		if(currentPage == null) currentPage = "1";
		if(search == null || search.equals("")){
			//검색하지 않고 전체목록에서 페이지를 넘긴경우
			noticeList = dao.getAllJapanese();
			...
			for(int i = 0; i < Integer.parseInt(currentPage); i++) {
				if(noticeList.size() > i*20) {
					for(int j = i*20; j < (i*20)+20 && j < noticeList.size(); j++) {
						...
						countList.add(commentDAO.countComment("japanese", noticeList.get(j).getNum()));
					}
				}
				if(i != Integer.parseInt(currentPage) - 1) {
					currentList = new ArrayList<JapaneseDTO>();
					countList = new ArrayList<Integer>();
				}
			}
			
			totalPage = String.valueOf(total);
			request.setAttribute("japaneseList", noticeList);
			request.setAttribute("currentList", currentList);
			request.setAttribute("countList", countList);
		}else{
			//검색후 다음페이지로 넘긴경우
			noticeList = dao.searchJapanese(search);
		...
		resURL = "/pages/japanese/japanese.jsp";
		return resURL;
	}
